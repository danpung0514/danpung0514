# Hi there, I'm Yunje Go 👋

## 안녕하세요, 문제를 해결하는 백엔드 개발자 고윤제입니다.

**"사용자 경험(UX)과 시스템 안정성을 동시에 고민하는 개발자"**
- 에이콘 아카데미 강남 **Java기반 풀스택 개발자 양성과정(6개월)**을 수료하였습니다.
- **Full-Stack 역량**을 바탕으로 시스템의 전체적인 흐름(End-to-End)을 설계하고 구현하는 것을 즐깁니다.
- **CI/CD 자동화 및 인프라 구축**을 통해 개발 효율성을 높이는 과정에 관심이 많습니다.

---
### 🏆 Awards
- **에이콘 아카데미 강남점** - Java기반 풀스택 개발자 양성과정 **우수상(1명)**
---
### 🛠 Tech Stack

| Category | Skills |
| :--- | :--- |
| **Backend** | ![Java](https://img.shields.io/badge/java-%23ED8B00.svg?style=flat-square&logo=openjdk&logoColor=white) ![Spring](https://img.shields.io/badge/spring-%236DB33F.svg?style=flat-square&logo=spring&logoColor=white) ![MyBatis](https://img.shields.io/badge/MyBatis-000000?style=flat-square&logo=MyBatis&logoColor=white) |
| **Database** | ![Oracle](https://img.shields.io/badge/Oracle-F80000?style=flat-square&logo=oracle&logoColor=white) |
| **DevOps** | ![AWS](https://img.shields.io/badge/AWS-%23FF9900.svg?style=flat-square&logo=amazon-aws&logoColor=white) ![Docker](https://img.shields.io/badge/docker-%230db7ed.svg?style=flat-square&logo=docker&logoColor=white) ![Nginx](https://img.shields.io/badge/nginx-%23009639.svg?style=flat-square&logo=nginx&logoColor=white) ![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=flat-square&logo=github-actions&logoColor=white) |
| **Frontend** | ![TypeScript](https://img.shields.io/badge/typescript-%23007ACC.svg?style=flat-square&logo=typescript&logoColor=white) ![React](https://img.shields.io/badge/react-%2320232a.svg?style=flat-square&logo=react&logoColor=%2361DAFB) ![JavaScript](https://img.shields.io/badge/javascript-%23323330.svg?style=flat-square&logo=javascript&logoColor=%23F7DF1E) |

---


### 📂 Main Project

#### [🏠 Staylog](https://github.com/danpung0514/Staylog_Backend)
- **설명**: 리뷰형 커뮤니티 기반 숙소 예약 웹 서비스
- **역할**: 프로젝트 팀장
- **주요 기여**: 
  - **CI/CD**: GitHub Actions & AWS EC2 연계 배포 자동화 파이프라인 구축
  - **성능 최적화**: N+1 쿼리 해결(Batch Loading), 비관적 잠금(Pessimistic Locking)을 통한 동시성 제어
  - **Full-Stack**: 숙소/객실/이미지 관리 도메인 End-to-End 개발
- **배포 URL**: [https://staylog.store](https://staylog.store)

---

### 📫 Contact
- **Email**: `kkokko514@naver.com`
- **GitHub**: [github.com/danpung0514](https://github.com/danpung0514)
